...
async def run_simulation(request: SimulationRequest) -> Dict[str, Any]:
    ...
    try:
        logger.info(f"Running simulation for {request.symbol} at {datetime.fromtimestamp(request.entry_time/1000)}")
...
async def run_optimization(request: OptimizationRequest) -> Dict[str, Any]:
    ...
    try:
        logger.info(f"Running {request.optimization_type} optimization for {request.symbol}")
