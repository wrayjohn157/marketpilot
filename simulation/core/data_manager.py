...
class HistoricalDataManager:
    ...
    def load_klines(self, 
        ...
        try:
            ...
            logger.info(f"Successfully loaded {len(klines)} candles for {symbol} {timeframe}")
            return klines
    ...
    def _fetch_from_binance(self, symbol: str, timeframe: str, start_time: int, end_time: int) -> pd.DataFrame:
        ...
        while current_start < end_time:
            ...
            params['endTime'] = min(current_start + self._get_timeframe_ms(timeframe) * 1000, end_time)
    ...
    def _load_from_cache(self, symbol: str, timeframe: str, start_time: int, end_time: int) -> Optional[pd.DataFrame]:
        """Load data from cache if available"""
        cache_file = self.cache_dir / f"{symbol}_{timeframe}_{start_time}_{end_time}.parquet"
    ...
    def _save_to_cache(self, df: pd.DataFrame, symbol: str, timeframe: str, start_time: int, end_time: int):
        """Save data to cache"""
        cache_file = self.cache_dir / f"{symbol}_{timeframe}_{start_time}_{end_time}.parquet"
        try:
    ...
    def get_data_summary(self, symbol: str, timeframe: str, start_time: int, end_time: int) -> Dict[str, Any]:
        """Get summary of available data for a symbol/timeframe"""
        try:
            # Load a small sample to check availability
            sample_end = start_time + self._get_timeframe_ms(timeframe) * 10  # 10 candles
            df = self.load_klines(symbol, timeframe, start_time, sample_end, use_cache=True)
            
            if df.empty:
                return {"available": False, "reason": "No data available"}
            
            # Get full range info
            full_df = self.load_klines(symbol, timeframe, start_time, end_time, use_cache=True)
