...
class ThreeCommasAPI:
    ...
    def _make_request(self, path: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
...
def simulate_new_avg_price(current_avg: float, current_volume: float, 
                          new_price: float, new_volume: float) -> float:
...
def get_macd_lift(prices: List[float], fast: int = 12, slow: int = 26, signal: int = 9) -> float:
    """Calculate MACD lift.
...
def save_entry_score_to_redis(symbol: str, deal_id: int, score_data: Dict[str, Any]) -> bool:
