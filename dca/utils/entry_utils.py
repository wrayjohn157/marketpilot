...
def get_rsi_slope(symbol, tf="15m", window=3):
    try:
        ...
        #print(f"[DEBUG] RSI slope for {symbol}: {rsi_series.iloc[-window]} → {rsi_series.iloc[-1]} = {slope}")
...
def get_macd_lift(symbol, tf="15m", window=3):
    try:
        ...
        #print(f"[DEBUG] MACD lift for {symbol}: {macd_series.iloc[-window]} → {macd_series.iloc[-1]} = {lift}")
        return round(lift, 5)
    except Exception as e:
