...
class ProfitabilityAnalyzer:
    ...
    def _calculate_profitability_score(self, df: pd.DataFrame) -> float:
        ...
        avg_volume = df["volume"].mean()
        volume_score = max(0, 100 - abs(avg_volume - 100) * 0.5)  # Optimal around 100 USDT
        ...
        # Weighted average
        weights = [0.25, 0.20, 0.15, 0.25, 0.15]  # Confidence and recovery most important
        weighted_score = sum(factor[1] * weight for factor, weight in zip(factors, weights))
    ...
    def _generate_recommendations(self, df: pd.DataFrame) -> List[str]:
        ...
        rescue_rate = len(df[df["decision"] == "rescue"]) / len(df) * 100
        if rescue_rate < 20:
            recommendations.append("Low rescue rate - consider lowering confidence thresholds")
        elif rescue_rate > 60:
            recommendations.append("High rescue rate - consider raising confidence thresholds")
        ...
        avg_recovery = df["recovery_odds"].mean()
        if avg_recovery < 0.5:
            recommendations.append("Low recovery odds - review recovery prediction model")
    ...
    def analyze_by_symbol(self, days: int = 7) -> Dict[str, Dict]:
        ...
        for symbol in df["symbol"].unique():
            ...
            symbol_analysis[symbol] = {
                ...
                "rescue_rate": len(symbol_df[symbol_df["decision"] == "rescue"]) / len(symbol_df) * 100,
