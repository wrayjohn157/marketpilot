...
class TechFilterMockGenerator:
    ...
    def get_active_trades(self) -> List[Dict]:
        ...
        try:
            response = requests.get(f"{self.api_base_url}/dca-trades-active", timeout=10)
        ...
        try:
            ...
            for trade in trades:
                try:
                    ...
                    logger.info(f"✅ {symbol}: RSI={indicators['indicators']['rsi']}, "
                              f"MACD={indicators['indicators']['macd']:.4f}, "
                              f"ADX={indicators['indicators']['adx']}, "
                              f"Score={tech_score:.3f}")
                    
                except Exception as e:
                    logger.error(f"Error processing trade {trade.get('deal_id', 'unknown')}: {e}")
                    continue
