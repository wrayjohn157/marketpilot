...
class ForkScorer:
    ...
    def _get_btc_multiplier(self) -> float:
        ...
        try:
            btc_price = float(self.redis.get("BTC_1h_latest_close") or 0)
            btc_ema50 = float(self.redis.get("BTC_1h_EMA50") or 0)
            btc_rsi = float(self.redis.get("BTC_15m_RSI14") or 50)
            btc_adx = float(self.redis.get("BTC_1h_ADX14") or 0)
