...
class TechFilter:
    ...
    def filter_symbol(
        ...
        thresholds = self.thresholds.get(
            self.config.btc_condition, self.thresholds["neutral"]
        )
    ...
    async def execute_trade(self, trade: Trade) -> bool:
        ...
        try:
            ...
            if success1 or success2:
                logging.info(
                    f"✅ Executed trade: {trade.symbol} (Score: {trade.adjusted_score:.3f})"
                )
                return True
            else:
    ...
    async def process_symbols(self, symbols: List[str]) -> List[Trade]:
        ...
        for symbol in symbols:
            try:
                ...
                passed, reason, tech_score = self.tech_filter.filter_symbol(
                    symbol, indicators
                )
                if not passed:
                ...
                passed, reason, fork_score, subscores = self.fork_scorer.score_symbol(
                    symbol, indicators
                )
                if not passed:
