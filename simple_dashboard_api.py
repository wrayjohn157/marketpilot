...
def get_active_trades():
    ...
    try:
        ...
        response = make_3commas_request(f"/public/api/ver1/deals?scope=active&bot_id={bot_id}")
        
        if response.status_code == 200:
            active_deals = response.json()
            logger.info(f"Successfully fetched {len(active_deals)} active deals for bot {bot_id} from 3Commas")
        else:
            logger.warning(f"3Commas API returned {response.status_code}, using mock data")
        ...
        for deal in active_deals:
            ...
            trades.append({
                ...
                "open_pnl": float(deal.get('usd_final_profit', 0)) if deal.get('usd_final_profit') else 0,
                "open_pnl_pct": float(deal.get('final_profit_percentage', 0)) if deal.get('final_profit_percentage') else 0,
                "current_price": float(deal.get('current_price', 0)) if deal.get('current_price') else 0,
                "entry_price": float(deal.get('bought_average_price', 0)) if deal.get('bought_average_price') else 0,
                "volume": float(deal.get('bought_volume', 0)) if deal.get('bought_volume') else 0,
...
def get_3commas_metrics():
    ...
    try:
        ...
        # Get active deals
        deals_response = make_3commas_request(f"/private/api/ver1/deals?account_id={account_id}")
...
def get_ml_confidence():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch ML confidence: {e}")
