...
class IntegrationGapFixer:
    ...
    def fix_mixed_config_usage(self) -> int:
        ...
        for root, dirs, files in os.walk(self.workspace_root):
            ...
            dirs[:] = [d for d in dirs if not d.startswith('config_migration_backup') and not d.startswith('redis_migration_backup')]
            ...
            try:
                ...
                if content != original_content:
                    with open(file_path, 'w', encoding='utf-8') as f:
                        f.write(content)
                    logger.info(f"✅ Fixed mixed config usage in {file_path.relative_to(self.workspace_root)}")
                    fixed_count += 1
    ...
    def fix_redis_usage(self) -> int:
        ...
        fixes = [
            ...
            (r'redis_client = redis\.Redis\([^)]*\)', 'redis_client = get_redis_manager()'),
        ...
        for file_path in files_to_fix:
            try:
                ...
                if 'get_redis_manager()' in content and 'from utils.redis_manager import get_redis_manager' not in content:
                    ...
                    if import_index >= 0:
                        lines.insert(import_index + 1, 'from utils.redis_manager import get_redis_manager')
                    else:
                        lines.insert(0, 'from utils.redis_manager import get_redis_manager')
                ...
                if content != original_content:
                    with open(file_path, 'w', encoding='utf-8') as f:
                        f.write(content)
                    logger.info(f"✅ Fixed Redis usage in {file_path.relative_to(self.workspace_root)}")
                    fixed_count += 1
        ...
        for file_path in files_to_fix:
            try:
                ...
                if content != original_content:
                    with open(file_path, 'w', encoding='utf-8') as f:
                        f.write(content)
                    logger.info(f"✅ Fixed missing imports in {file_path.relative_to(self.workspace_root)}")
                    fixed_count += 1
