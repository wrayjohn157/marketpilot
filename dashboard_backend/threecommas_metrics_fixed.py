...
class ThreeCommasAPI:
    ...
    def _make_request(self, path: str, query: str = "", method: str = "GET") -> Dict[str, Any]:
...
def calculate_open_pnl(active_deals: List[Dict[str, Any]]) -> Tuple[float, List[Dict[str, Any]]]:
    ...
    for deal in active_deals:
        try:
            ...
            drawdown_pct = ((entry_price - current_price) / entry_price) * 100 if entry_price > 0 and current_price < entry_price else 0.0
            drawdown_usd = (entry_price - current_price) * coins_bought if entry_price > 0 and current_price < entry_price else 0.0
            ...
            logger.warning(f"Failed to calculate PnL for deal {deal.get('id', 'unknown')}: {e}")
            continue
    
    return total_open_pnl, deals_info

def calculate_closed_deals_stats(finished_deals: List[Dict[str, Any]]) -> Tuple[float, float, int, float]:
    ...
    for deal in finished_deals:
        try:
            ...
            logger.warning(f"Failed to process closed deal {deal.get('id', 'unknown')}: {e}")
            continue
...
def get_3commas_metrics() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"Successfully fetched 3Commas metrics: {len(open_deals_info)} active deals, {total_deals} total deals")
        return response
