...
dca_successful_attempts_total = Counter(
    "dca_successful_attempts_total", "Total DCA successful attempts", registry=registry
)

active_trades_gauge = Gauge(
    "active_trades_total", "Number of active trades", registry=registry
)

trading_volume_gauge = Gauge(
    "trading_volume_usd", "Trading volume in USD", registry=registry
)
...
ml_model_accuracy_gauge = Gauge(
    "ml_model_accuracy", "ML model accuracy", ["model_type"], registry=registry
)
...
redis_connection_errors_total = Counter(
    "redis_connection_errors_total", "Total Redis connection errors", registry=registry
)
...
fork_score_gauge = Gauge(
    "fork_score", "Current fork score", ["symbol"], registry=registry
)

btc_sentiment_gauge = Gauge(
    "btc_sentiment_score", "BTC sentiment score", registry=registry
)

market_volatility_gauge = Gauge(
    "market_volatility", "Market volatility index", registry=registry
)
...
def record_trading_error(error_type, service):
    ...
    trading_errors_total.labels(error_type=error_type, service=service).inc()
