...
def run_once():
    ...
    for base in symbols:
        ...
        for tf in TIMEFRAMES:
            ...
            try:
                r.set(f"{base.upper()}_{tf}_RSI14", indicators["RSI14"])
                r.set(f"{base.upper()}_{tf}_StochRSI_K", indicators["StochRSI_K"])
                r.set(f"{base.upper()}_{tf}_StochRSI_D", indicators["StochRSI_D"])
            except Exception as e:
