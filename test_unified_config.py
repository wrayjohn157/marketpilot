...
def test_environment_detection():
    ...
    assert (
        env_info.environment in Environment
    ), f"Invalid environment: {env_info.environment}"
    assert (
        env_info.base_path.exists() or env_info.is_development
    ), f"Base path does not exist: {env_info.base_path}"
...
n🧪 Testing Smart Defaults..."
    ...
    for config_name in required_configs:
        ...
        assert isinstance(
            config_defaults[config_name], dict
        ), f"Config not a dict: {config_name}"
...
n🧪 Testing Convenience Functions..."
    ...
    ), "get_config_manager not returning UnifiedConfigManager"
...
n🧪 Testing Environment-Specific Paths..."
    ...
    for env in environments:
        ...
        assert (
            defaults["base"] == expected_base
        ), f"Wrong base path for {env.value}: {defaults['base']}"
