...
class Market7Refactorer:
                ...
                if params.strip():
                    ...
                    for param in param_parts:
                        if "=" in param:
                            ...
                            typed_params.append(
                                f"{name.strip()}: Any = {default.strip()}"
                            )
                        else:
        ...
        try:
            ...
            for imp in imports:
                if any(
                    ...
                    module in imp
                    for module in ["pandas", "numpy", "requests", "redis", "yaml", "ta"]
                ):
    ...
    def refactor_directory(
        self, directory: Path, exclude_dirs: List[str] = None
    ) -> None:
        ...
        for file_path in directory.rglob("*.py"):
            ...
            if any(results.values()):
                logger.info(
                    f"Modified {file_path}: {[k for k, v in results.items() if v]}"
                )
