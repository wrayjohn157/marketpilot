...
const DcaStrategyBuilder = () => {
      ...
      .then(res => {
        if (Array.isArray(res.data.series)) {
          ...
          const mapped = res.data.series.map(d => ({
            timestamp: Number(d.timestamp),  // 🔥 Ensure numeric
            open: d.open,
            high: d.high,
            low: d.low,
            close: d.close,
  ...
  console.log("Rendering DcaStrategyBuilder with series:", series?.length);
  return (
    <div className="p-6 text-white">
      ...
      <div className="mt-6 w-full" style={{ height: '400px' }}>
        <CandleChart
          ...
          width={800}
          height={400}
          selectedIndex={entryTime !== null
            ? series.findIndex(c => c.timestamp === entryTime)
            : -1}
          onCandleClick={(timestampSec) => {
            console.log("Candle clicked →", timestampSec);
            // Convert seconds to ms
            setEntryTime(timestampSec * 1000);
          }}
          dcaMarkers={dcaResults}
        />
      </div>
      ...
      {entryTime !== null && (
        <div className="mt-2 text-sm text-gray-300">
          Entry Time: {new Date(entryTime).toLocaleString("en-US", {
            timeZone: "UTC",
            calendar: "gregory",
            year: "numeric",
            month: "numeric",
            ...
            {dcaResults.map((step, i) => (
              <div key={i} className="border-b border-gray-700 pb-1">
                ...
                <span className="text-green-300">
                  {step.time
                    ? new Date(Number(step.time)).toLocaleString("en-US", {
                        timeZone: "UTC",
                        calendar: "gregory",
                        year: "numeric",
                        month: "numeric",
