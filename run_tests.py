...
def main():
    ...
    if args.unit or args.all:
        ...
        if args.coverage:
            cmd.extend(["--cov=core", "--cov=dca", "--cov=fork", "--cov=lev", "--cov-report=html"])
        if not args.verbose:
            cmd.append("-q")
    ...
    if args.lint or args.all:
        if not run_command(["python", "-m", "flake8", "core", "dca", "fork", "lev", "utils"], "Flake8 Linting"):
            success = False
    
    # Formatting check
    if args.format or args.all:
        if not run_command(["python", "-m", "black", "--check", "core", "dca", "fork", "lev", "utils"], "Black Formatting Check"):
            success = False
        
        if not run_command(["python", "-m", "isort", "--check-only", "core", "dca", "fork", "lev", "utils"], "Import Sorting Check"):
            success = False
    
    # Type checking
    if args.type_check or args.all:
        if not run_command(["python", "-m", "mypy", "core", "dca", "fork", "lev", "utils"], "MyPy Type Checking"):
            success = False
