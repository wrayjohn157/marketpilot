...
def load_config(path: Path) -> tuple:
    ...
    if any(k in weights for k in key_map.keys()):
        logging.warning(
            "Detected spot-style weight keys in config; translating for lev scorer."
        )
        translated = {}
...
def btc_sentiment_multiplier(side: str = "long"):
    """
    Side-aware BTC kicker.
    Priority:
      1) If Redis has 'btc_condition' (bullish/neutral/bearish), use that.
      2) Else fall back to deriving from BTC_1h / BTC_15m blobs (if present).
    Honors options.use_btc_multiplier; returns 1.0 if disabled.
    """
    # Respect YAML option
    if not OPTIONS.get("use_btc_multiplier", True):
        logging.info("[BTC mult] disabled via config options.use_btc_multiplier=false")
        return 1.0

    # --- 1) Prefer explicit btc_condition ---
    cond = None
    try:
        cond = r.get_cache("cache:btc_condition") if r else None
    except Exception:
        cond = None

    if cond:
        cond = str(cond).strip().lower()
        # Side-aware mapping: long gets boost on bullish; short gets opposite
        if side == "long":
            table = {"bullish": 1.10, "neutral": 1.00, "bearish": 0.90}
        else:  # short
            table = {"bullish": 0.95, "neutral": 1.00, "bearish": 1.10}
        mult = table.get(cond, 1.00)
        logging.info(f"[BTC mult] source=btc_condition value='{cond}' side={side} -> {mult:.2f}")
        return mult

    # --- 2) Fallback: derive from BTC_1h / 15m (if snapshots exist) ---
    price = extract_float(r.get_cache("indicators:BTC:1h:latest_close")) if r else 0.0
    ema50 = extract_float(r.get_cache("indicators:BTC:1h:EMA50")) if r else 0.0
    rsi = extract_float(r.get_cache("indicators:BTC:15m:RSI14")) if r else 0.0
    adx = extract_float(r.get_cache("indicators:BTC:1h:ADX14")) if r else 0.0

    mult = 1.0
    long_bias = (price > ema50) if (price and ema50) else False
    if side == "short":
        long_bias = not long_bias  # invert for shorts
    ...
    if side == "long" and rsi and rsi < 35:
        mult -= 0.05
    if side == "short" and rsi and rsi > 65:
        mult -= 0.05

    mult = max(0.8, min(mult, 1.2))
    logging.info(
        f"[BTC mult] source=fallback(BTC_1h/15m) side={side} -> {mult:.2f} "
        f"(price={price}, ema50={ema50}, adx={adx}, rsi15={rsi})"
    )
    return mult
...
def compute_subscores(symbol, side: str = "long"):
    data = r.get_cache(f"{symbol.upper()}_1h") if r else None
    ...
    rsi = extract_float(r.get_cache(f"{symbol.upper()}_15m_RSI14")) if r else 0.0
    k = extract_float(r.get_cache(f"{symbol.upper()}_15m_StochRSI_K")) if r else 0.0
    d = extract_float(r.get_cache(f"{symbol.upper()}_15m_StochRSI_D")) if r else 0.0

    current_vol = extract_float(r.get_cache(f"{symbol.upper()}_15m_volume")) if r else 0.0
    sma9_vol = extract_float(r.get_cache(f"{symbol.upper()}_15m_volume_sma9")) if r else 0.0
    ...
    subscores = {
        ...
        "volume_penalty": (
            ...
            if OPTIONS.get("use_volume_penalty", True)
            else 0.0
        ),
...
def main() -> Any:
    ...
    parser.add_argument(
        "--backtest", type=str, default=str(DEFAULT_BACKTEST_CANDIDATES_FILE)
    )
    parser.add_argument("--redis-prefix", type=str, default=REDIS_PREFIX)
    parser.add_argument(
        "--no-redis", action="store_true", help="Run without connecting to Redis"
    )
    ...
    parser.add_argument(
        "--input",
        type=str,
        help="Path to JSON list of symbols (e.g., lev/data/perps/lev_symbols.json)",
    )
    ...
    # --- Per-side config overrides (min_score & weights) ---
    min_long = full_cfg.get("min_score_long")
    min_short = full_cfg.get("min_score_short")
    weights_long = full_cfg.get("weights_long")
    weights_short = full_cfg.get("weights_short")

    if args.side == "long" and (min_long is not None or isinstance(weights_long, dict)):
        if min_long is not None:
            MIN_SCORE = min_long
        if isinstance(weights_long, dict):
            WEIGHTS = weights_long
    elif args.side == "short" and (min_short is not None or isinstance(weights_short, dict)):
        if min_short is not None:
            MIN_SCORE = min_short
        if isinstance(weights_short, dict):
            WEIGHTS = weights_short

    logging.info(f"Using side={args.side} MIN_SCORE={MIN_SCORE} with {len(WEIGHTS)} weights")
    ...
    if backtest_path.suffix == "":
        ...
        backtest_path = backtest_path.with_name(
            f"fork_backtest_candidates_{args.side}.json"
        )
    ...
    for s in raw_symbols:
        ...
        if pair in universe_pairs:
            symbols.append(
                pair.replace("USDT", "")
            )  # keep bare ticker for downstream lookups
    symbols = sorted(list(dict.fromkeys(symbols)))

    if not symbols:
        logging.warning(
            "No symbols after filtering to leverage universe; nothing to score."
        )
