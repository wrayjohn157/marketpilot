...
def send_btc_trade():
    ...
    start_data = {"bot_id": int(bot_id)}

    response = make_request(
        "POST", f"/public/api/ver1/bots/{bot_id}/start_new_deal", start_data
    )
    ...
    if response.status_code == 200:
        ...
        print(
            "⚠️  Bot cannot start new deal (possibly already running or conditions not met)"
        )
    ...
    if response.status_code == 200:
        ...
        print(
            f"⚠️  Could not get account balance: {response.status_code} - {response.text}"
        )

    print("\n💡 Note: This is a paper trading bot.")
    print(
        "💡 The bot will execute trades based on its configured strategy and market conditions."
    )
