...
class TechFilterDataCollector:
    def __init__(self):
        ...
        self.session.headers.update({
            'User-Agent': 'MarketPilot-TechFilter/1.0'
        })
        
    def fetch_klines(self, symbol: str, interval: str, limit: int = 100) -> pd.DataFrame:
    ...
    def calculate_indicators(self, df: pd.DataFrame) -> dict:
        ...
        try:
            ...
            indicators["ema_200"] = float(ema_200.iloc[-1]) if not ema_200.empty else None
            ...
            # MACD
            macd = MACD(df["close"])
            indicators["macd"] = float(macd.macd().iloc[-1]) if not macd.macd().empty else None
            indicators["macd_signal"] = float(macd.macd_signal().iloc[-1]) if not macd.macd_signal().empty else None
            indicators["macd_histogram"] = float(macd.macd_diff().iloc[-1]) if not macd.macd_diff().empty else None
            ...
            indicators["stoch_rsi_k"] = float(stoch_rsi.stochrsi_k().iloc[-1]) if not stoch_rsi.stochrsi_k().empty else None
            indicators["stoch_rsi_d"] = float(stoch_rsi.stochrsi_d().iloc[-1]) if not stoch_rsi.stochrsi_d().empty else None
