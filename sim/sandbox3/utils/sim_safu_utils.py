...
def calculate_volume_drop_pct(klines: list) -> float:
    if len(klines) < 21:
        return None
    volumes = [float(k[5]) for k in klines[-21:-1]]  # 20 bars before most recent
    ma20 = sum(volumes) / len(volumes)
    last_volume = float(klines[-1][5])
    drop_pct = (last_volume - ma20) / ma20 * 100
    return round(max(0.0, -drop_pct), 2)
    ...
    if kline_file.exists():
        with open(kline_file) as f:
            try:
                ...
                recent_klines = recent_klines[-21:]  # For VWAP and volume MA20
                if recent_klines:
                    vwap = calculate_vwap(recent_klines[-10:])
                    volume_drop_pct = calculate_volume_drop_pct(recent_klines)
            except Exception:
                pass

    if not best:
        # fallback: compute indicators from klines if available
        if kline_file.exists():
            try:
                with open(kline_file) as f:
                    klines = json.load(f)
