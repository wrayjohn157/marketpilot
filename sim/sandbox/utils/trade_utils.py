...
def load_price_at_entry(symbol: str, entry_time: int, tf: str = "1h") -> float:
    ...
    klines = load_forward_klines_cross_days(symbol, tf, entry_time)
    if klines is None:
    ...
    valid = [
        k
        for k in klines
        if isinstance(k, dict) and _to_int_timestamp(k.get("timestamp")) <= entry_time
    ]
    if not valid:
        # fallback: try to find the nearest earlier kline by sorting
        sorted_klines = sorted(
            [k for k in klines if isinstance(k, dict)],
            key=lambda x: _to_int_timestamp(x["timestamp"]),
        )
        for k in reversed(sorted_klines):
            if _to_int_timestamp(k["timestamp"]) <= entry_time:
                ...
                return k["close"]
        print(
            f"🛠 Fallback failed. Kline timestamps seen: [{[_to_int_timestamp(k['timestamp']) for k in sorted_klines]}]"
        )
        return None
