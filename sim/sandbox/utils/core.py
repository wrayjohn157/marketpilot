...
def load_local_klines(symbol, tf="15m", lookback=100, base_time=None):
    ...
    :param tf: timeframe string like '15m'
    :param lookback: number of candles to retrieve
    ...
    file_path = SNAPSHOT_DIR / date_str / f"{symbol}_{tf}_klines.json"

    if not file_path.exists():
        print(f"[WARN] Snapshot not found: {file_path}")
        return None
    ...
    try:
        ...
        if base_time:
            df = df[df["timestamp"] <= base_time].tail(lookback)
        else:
            df = df.tail(lookback)
        return df
...
def compute_indicators(symbol: str, tf: str = "15m", entry_time: int = None) -> dict:
    if entry_time:
        df = load_local_klines(symbol, tf=tf, lookback=100, base_time=entry_time)
    else:
...
def simulate_dca_step(trade, config):
    ...
    if entry_time:
        btc_df = load_local_klines("BTCUSDT", tf="1h", lookback=251, base_time=entry_time)
    else:
