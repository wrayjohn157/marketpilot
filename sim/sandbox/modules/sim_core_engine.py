...
def compute_indicators(symbol: str, tf: str = "15m", entry_time: int = None) -> dict:
    if entry_time:
        raw = load_forward_klines_cross_days(symbol, tf, start_time=entry_time)
...
def simulate_dca_step(trade: Any, config: Any) -> Any:
    ...
    logger.debug(f"[🔍] Health inputs: {json.dumps(trade_features, indent=2)}")
    health = evaluate_trade_health(trade_features)
    health_score = health["health_score"]
    health_status = health["health_status"]
    logger.debug(f"[🧠] Health result: Score {health_score}, Status {health_status}")

    if entry_time:
        raw_btc = load_forward_klines_cross_days("BTCUSDT", tf, start_time=entry_time)
        btc_df = pd.DataFrame(raw_btc) if isinstance(raw_btc, list) else pd.DataFrame(raw_btc)
    else:
        btc_df = fetch_binance_klines("BTCUSDT", interval="1h", limit=251)

    # 🩹 Fix unnamed columns if needed
    ...
    # 🩹 Ensure all values are numeric for model input
...
def run_dca_simulation(
    ...
    # Use only the CLI timeframe for all indicator loads
    config["timeframe"] = tf
    ...
    for idx, kline in enumerate(klines):
        ...
        # detailed per-step output now logged at DEBUG level instead of printing
        logger.debug(json.dumps(result, indent=2))

    print(f"[✅] Simulation complete — evaluated {len(results)} DCA bars from entry_time.")
    return results
