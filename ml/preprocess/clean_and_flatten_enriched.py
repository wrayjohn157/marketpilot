...
REQUIRED_TRADE_FIELDS = [
    ...
    "symbol", "pnl_pct", "safety_orders", "status"
]
...
def is_valid_entry(entry):
    ...
    for val in fork["raw_indicators"].values():
        if val is None:
            return False
...
def main():
    parser = argparse.ArgumentParser(description="Clean and flatten enriched trade files for ML.")
    parser.add_argument("--date", help="Target date (YYYY-MM-DD)")
    parser.add_argument("--all", action="store_true", help="Process all enriched_data.jsonl folders")
    args = parser.parse_args()

    if args.all:
        total = 0
        for folder in sorted(ENRICHED_ROOT.glob("2025-05-*")):
            date_str = folder.name
            if (folder / "enriched_data.jsonl").exists():
                total += process_file(date_str)
        print(f"\n📦 All days processed. Total cleaned entries: {total}")
    else:
