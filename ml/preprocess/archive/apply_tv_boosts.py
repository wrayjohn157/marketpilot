...
def normalize(s):    return s.upper().rstrip("USDT")
def load_jsonl(p):   return [json.loads(l) for l in p.open() if l.strip()] if p.exists() else []
def dtms(ms):        return datetime.fromtimestamp(ms/1000, tz=timezone.utc)
...
def main():
    ...
    for t in rem:
        sym, e_dt = normalize(t["symbol"]), datetime.strptime(t["entry_time"], "%Y-%m-%dT%H:%M:%SZ").replace(tzinfo=timezone.utc)
        best_tv, tb = None, float("inf")

        for tv in tvs:
            if not tv.get("pass"): continue
            if normalize(tv.get("symbol", "")) != sym: continue
            try:
        ...
        match_found = False
        for f in forks:
            if normalize(f.get("symbol", "")) != sym: continue
            try:
                f_score = Decimal(str(round(f["score"], 4)))
                fdt = dtms(f["timestamp"])
                if abs(f_score - tv_score) <= SCORE_TOLERANCE and abs((fdt - tvdt).total_seconds()) <= FORK_GRACE_S:
                    f2 = dict(f)
                    f2.update({
        ...
        if not match_found:
            # === DEBUG LOGGING ===
            print(f"[❌ NO FORK MATCH] {sym} | trade @ {e_dt.isoformat()} | TV score {tv_score} @ {tvdt.isoformat()}")
            still.append(t)

    with open(ENRICH/d/"tv_boosted.jsonl","w") as f:
        for r in boosted: f.write(json.dumps(r)+"\n")
    with open(ENRICH/d/"still_unmatched.jsonl","w") as f:
        for r in still:   f.write(json.dumps(r)+"\n")
