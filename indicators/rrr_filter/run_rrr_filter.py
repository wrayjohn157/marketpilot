...
if __name__ == "__main__":
    ...
    if klines_raw:
        ...
        print(f"📈 Pulled {len(klines)} klines from Redis for {symbol}")
    else:
        print(f"❌ No klines found for {symbol} (Redis key: {kline_key})")
        klines = []

    if not klines or len(klines) < 20:
        print(f"⚠️ Insufficient klines for {symbol}, skipping RRR filter")
        exit()
