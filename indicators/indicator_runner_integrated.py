...
from utils.unified_indicator_system import UnifiedIndicatorManager, IndicatorConfig
...
class IntegratedIndicatorManager:
            ...
            for timeframe in self.config.timeframes:
                try:
                    ...
                    if indicators:
                        ...
                        logger.warning(f"⚠️ No indicators calculated for {symbol} {timeframe}")
                        results["errors"].append(f"No indicators for {timeframe}")
                    
                except Exception as e:
                    logger.error(f"Failed to calculate {symbol} {timeframe} indicators: {e}")
    ...
    def get_indicator_data(self, symbol: str, timeframe: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if indicators:
                ...
                logger.debug(f"Calculated and cached indicators for {symbol} {timeframe}")
                return indicators
    ...
    def run_indicator_cycle(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.redis_manager.set_counter("indicators_calculated", results["successful"])
            self.redis_manager.set_counter("indicators_timestamp", int(datetime.utcnow().timestamp()))
    ...
    def _save_results(self, results: Dict[str, Any]) -> None:
        ...
        try:
            results_file = get_path("snapshots") / f"indicators_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.json"
