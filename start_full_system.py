...
class FullSystemManager:
    ...
    def start_full_system(self):
        ...
        try:
            subprocess.run(['sudo', 'docker-compose', '-f', 'deploy/docker-compose-minimal.yml', 'up', '-d'], check=True)
            time.sleep(5)
            subprocess.run(['sudo', 'docker-compose', '-f', 'deploy/docker-compose-dashboard.yml', 'up', '-d'], check=True)
    ...
    def stop_all(self):
        ...
        try:
            subprocess.run(['sudo', 'docker-compose', '-f', 'deploy/docker-compose-dashboard.yml', 'down'])
            subprocess.run(['sudo', 'docker-compose', '-f', 'deploy/docker-compose-minimal.yml', 'down'])
