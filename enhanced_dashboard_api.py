...
def get_3commas_credentials():
    ...
    try:
        with open('/home/signal/marketpilot/config/credentials/3commas_default.json', 'r') as f:
            return json.load(f)
...
def make_3commas_request(path: str, params: Dict = None) -> requests.Response:
    ...
    try:
        ...
        if response.status_code != 200:
            logger.warning(f"3Commas API returned {response.status_code}: {response.text[:200]}")
...
def get_active_deals(bot_id: str, use_cache: bool = True) -> List[Dict]:
    ...
    # Fetch from 3Commas
    try:
        response = make_3commas_request("/public/api/ver1/deals", {
            "scope": "active",
            "bot_id": bot_id
        })
...
def get_finished_deals(bot_id: str, limit: int = 100, use_cache: bool = True) -> List[Dict]:
    """
...
def calculate_performance_metrics(finished_deals: List[Dict]) -> Dict:
    ...
    for deal in finished_deals:
        ...
        if deal.get('closed_at'):
            try:
                closed_date = datetime.fromisoformat(deal['closed_at'].replace('Z', '+00:00')).date()
...
def enrich_active_deals(active_deals: List[Dict]) -> List[Dict]:
    ...
    for deal in active_deals:
        ...
        bought_volume = float(deal.get('bought_volume', 0))  # This is already in USDT (spent amount)
        bought_amount = float(deal.get('bought_amount', 0))  # This is the crypto quantity
        ...
        if created_at:
            try:
                ...
                trade_age_hours = (datetime.utcnow().replace(tzinfo=created_time.tzinfo) - created_time).total_seconds() / 3600
            except:
                pass
        ...
        enriched_deal = {
            ...
            "spent_amount": round(spent_amount, 2),  # Now correctly using bought_volume (USDT spent)
...
def get_comprehensive_metrics():
    ...
    try:
        ...
        open_pnl = sum(float(deal.get('actual_usd_profit', deal.get('usd_final_profit', 0))) for deal in active_deals)
