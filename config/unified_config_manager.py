...
class SmartDefaults:
    ...
    def get_default_paths(environment: Environment, base_path: Path) -> Dict[str, Path]:
        ...
        defaults = {
            ...
            "final_fork_rrr_trades": base_path / "output" / "final_fork_rrr_trades.json",
            ...
            "fork_backtest_candidates": base_path / "output" / "fork_backtest_candidates.json",
            "dca_tracking_log": base_path / "dca" / "logs" / "dca_tracking" / "dca_fired.jsonl",
            ...
            "fork_trade_candidates": base_path / "output" / "fork_trade_candidates.json",
            ...
            "dca_tracking": base_path / "dca" / "logs" / "dca_tracking" / "dca_fired.jsonl",
        ...
        return {
            ...
            "unified_pipeline_config": {
                ...
                "tech_filter": {
                    ...
                    "thresholds": {
                        "neutral": {
                            "15m": {"qqe_min": 30, "qqe_max": 50, "rsi_range": [35, 65]},
            ...
            "ml_pipeline_config": {
                ...
                "models": {
                    "safu_exit": {
                        ...
                        "min_accuracy": 0.8,
                        "features": ["rsi", "stoch_rsi_k", "macd_histogram", "adx", "ema_distance"]
                    },
                    "recovery_odds": {
                        "enabled": True,
                        "retrain_interval": 86400,
                        "min_accuracy": 0.75,
                        "features": ["rsi", "stoch_rsi_k", "macd_histogram", "adx", "ema_distance", "volume_ratio"]
                    },
                    "confidence_score": {
                        "enabled": True,
                        "retrain_interval": 86400,
                        "min_accuracy": 0.7,
                        "features": ["rsi", "stoch_rsi_k", "macd_histogram", "adx", "ema_distance", "volume_ratio", "atr"]
                    },
                    "dca_spend": {
                        "enabled": True,
                        "retrain_interval": 86400,
                        "min_accuracy": 0.8,
                        "features": ["rsi", "stoch_rsi_k", "macd_histogram", "adx", "ema_distance", "volume_ratio", "atr", "price"]
                    },
                    "trade_success": {
                        "enabled": True,
                        "retrain_interval": 86400,
                        "min_accuracy": 0.75,
                        "features": ["rsi", "stoch_rsi_k", "macd_histogram", "adx", "ema_distance", "volume_ratio", "atr", "price", "btc_sentiment"]
                    }
                }
            }
        }
...
class ConfigValidator:
    ...
    def validate_paths(paths: Dict[str, Path]) -> List[str]:
        ...
        for key, path in paths.items():
            ...
            required_paths = ["base", "snapshots", "fork_history", "btc_logs", "live_logs", "models"]
    ...
    def _validate_dca_config(config: Dict[str, Any]) -> List[str]:
        ...
        for field in required_fields:
            if field not in config:
                ...
                issues.append(f"DCA config field '{field}' must be numeric: {type(config[field])}")
        
        # Validate score ranges
        if "min_score" in config and not (0 <= config["min_score"] <= 1):
            issues.append(f"DCA min_score must be between 0 and 1: {config['min_score']}")
        
        if "max_dca_attempts" in config and not (1 <= config["max_dca_attempts"] <= 10):
            issues.append(f"DCA max_dca_attempts must be between 1 and 10: {config['max_dca_attempts']}")
        ...
        if "weights" in config:
            for key, weight in config["weights"].items():
                if not isinstance(weight, (int, float)):
                    issues.append(f"TV Screener weight '{key}' must be numeric: {type(weight)}")
                elif not (-1 <= weight <= 1):
                    issues.append(f"TV Screener weight '{key}' must be between -1 and 1: {weight}")
        ...
        if "tech_filter" in config and "thresholds" in config["tech_filter"]:
            ...
            for condition, timeframes in thresholds.items():
                if not isinstance(timeframes, dict):
                    issues.append(f"Tech filter thresholds for '{condition}' must be a dict")
                    continue
                
                for tf, params in timeframes.items():
                    if not isinstance(params, dict):
                        issues.append(f"Tech filter thresholds for '{condition}.{tf}' must be a dict")
    ...
    def _validate_ml_pipeline_config(config: Dict[str, Any]) -> List[str]:
        ...
        if "models" in config:
            for model_name, model_config in config["models"].items():
                ...
                if "min_accuracy" in model_config and not (0 <= model_config["min_accuracy"] <= 1):
                    issues.append(f"ML model '{model_name}' min_accuracy must be between 0 and 1: {model_config['min_accuracy']}")
...
class UnifiedConfigManager:
    ...
    def _load_paths(self):
        ...
        # Get smart defaults
        defaults = SmartDefaults.get_default_paths(self.env_info.environment, self.env_info.base_path)
    ...
    def _ensure_required_paths(self):
        """Ensure all required paths exist"""
        required_paths = ["snapshots", "fork_history", "btc_logs", "live_logs", "models"]
    ...
    def _validate_all(self):
        ...
        for config_name, config_data in self.configs.items():
            ...
            self.validation_issues.extend([f"{config_name}: {issue}" for issue in config_issues])
        
        # Log validation issues
        if self.validation_issues:
            logging.warning(f"Config validation issues found: {len(self.validation_issues)}")
    ...
    def get_config(self, key: str) -> Dict[str, Any]:
        ...
        if key not in self.configs:
            raise KeyError(f"Config key '{key}' not found. Available keys: {list(self.configs.keys())}")
